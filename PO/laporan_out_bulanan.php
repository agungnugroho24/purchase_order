...
$year=$_POST['select2'];
    ...
    $this->Ln();
	$this->SetFont('Arial','B',14);
    $this->Cell(0,0.75,'LAPORAN PERMINTAAN BARANG BULANAN',0,0,'C');
    $this->Ln();
    $this->SetFont('Arial','B',11);
	$this->Cell(7);
    $this->Cell(2,0.75,'Periode : ',0,0,'L');
	$this->Cell(2.5,0.75, $bulan ,0,0,'C');
...
$query="SELECT a.id_brg, sum(a.jml_brg) as jml, c.nama_unit, b.id_brg, b.nama_brg FROM trx_barang a LEFT JOIN ms_barang b ON a.id_brg=b.id_brg LEFT JOIN ms_unit c ON a.id_unit=c.id_unit WHERE MONTHNAME(tgl)='$month' AND YEAR(tgl)='$year' GROUP BY a.id_brg, c.nama_unit ORDER BY a.id_brg";
