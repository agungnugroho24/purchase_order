...
$data_tanggal_mysql2 = jin_date_sql($data_tanggal_form2);
    ...
    $this->Ln();
	$this->SetFont('Arial','B',14);
    $this->Cell(0,0.75,'LAPORAN PERMINTAAN BARANG HARIAN',0,0,'C');
    $this->Ln();
    $this->SetFont('Arial','B',11);
	$this->Cell(5.5);
    $this->Cell(2,0.75,'Periode : ',0,0,'L');
	$this->Cell(2.5,0.75, $date1 ,0,0,'C');
	$this->Cell(1,0.75,'s/d',0,0,'C');
	$this->Cell(2.5,0.75, $date2 ,0,0,'C');
...
$i = 1;
  foreach($data as $row)
  {
    ...
    $this->Cell($w[2],0.75,$row[4],1,0,'C',$fill);
...
$query="SELECT a.id_brg, sum(a.jml_brg) as jml, c.nama_unit, b.id_brg, b.nama_brg FROM trx_barang a LEFT JOIN ms_barang b ON a.id_brg=b.id_brg LEFT JOIN ms_unit c ON a.id_unit=c.id_unit WHERE tgl BETWEEN '$data_tanggal_mysql1' AND '$data_tanggal_mysql2' GROUP BY a.id_brg, c.nama_unit ORDER BY a.id_brg";
