...
$year=$_POST['select1'];
    ...
    $this->Ln();
	$this->SetFont('Arial','B',14);
    $this->Cell(0,0.75,'LAPORAN PENGADAAN BARANG TAHUNAN',0,0,'C');
    $this->Ln();
    $this->SetFont('Arial','B',11);
	$this->Cell(8);
...
$i = 1;
  foreach($data as $row)
  {
    ...
    $this->Cell($w[0],0.75,$i,1,0,'C');
    $this->Cell($w[1],0.75,$row[0],1,0,'L');
    $this->Cell($w[2],0.75,$row[2],1,0,'C');
	$this->Cell($w[3],0.75,$row[1],1,0,'C');
    $this->Ln();
//    $fill = !$fill;
   $i++;
  }
...
$query="SELECT a.id_brg, sum(a.jml_brg) as jml, b.nama_brg FROM in_barang a LEFT JOIN ms_barang b ON a.id_brg=b.id_brg WHERE YEAR(tgl)='$year' GROUP BY b.id_brg";
